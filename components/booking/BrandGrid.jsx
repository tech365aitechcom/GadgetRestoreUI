...
const GearDecoration = () => (
  <svg viewBox="0 0 150 140" fill="white" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
...
export function CantFindBanner({ desktop = false }) {
  return (
    <div className={`cant-find-banner${desktop ? ' cant-find-banner-desktop' : ''}`}>
      <div style={{ position: 'relative', zIndex: 1, flex: 1 }}>
        <div className="cant-find-title">Can't find your model?</div>
        <p className="cant-find-desc">
          Our technicians can repair older legacy devices. Contact support for a custom quote.
        </p>
        <a href="tel:+919999999999" className="cant-find-cta">
...
export function TrustBadges() {
  const items = [
    { icon: '🛡️', title: 'Certified Technicians', sub: 'Expert repair professionals' },
    ...
    <div className="trust-row">
      {items.map(({ icon, title, sub }) => (
        <div key={title} className="trust-item">
          <div className="trust-icon" style={{ background: '#1a1a1a', fontSize: 18 }}>{icon}</div>
          <div>
  ...
  return (
    ...
    >
      <div>
        ...
        {!compact && (
          <div style={{ marginTop: 6, height: 3, width: 60, borderRadius: 3, background: 'rgba(255,255,255,0.12)' }}>
...
/* ══════════════════════════════════════════════════════════════════════════
   BRAND GRID
   ══════════════════════════════════════════════════════════════════════════ */
export default function BrandGrid({ brands = [], isLoading, onSelectBrand, selectedBrandId }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%' }}>
      ...
      <div className="search-input-row">
        ...
        />
        {search && (
          <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-content-text-secondary)', lineHeight: 1 }}>✕</button>
        )}
      </div>
      ...
      {!isLoading && filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '48px 16px', color: 'var(--color-content-text-secondary)' }}>
          ...
          <div style={{ fontWeight: 700, marginBottom: 6 }}>No brands found</div>
