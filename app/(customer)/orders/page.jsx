...
// ---------------------------------------------------------------------------
// Pure helper — extracted to reduce cognitive complexity of OrdersPage
// ---------------------------------------------------------------------------
function getDeviceName(brandRef, modelRef) {
  const brandName = brandRef?.name || ''
...
function getDateLabel(order) {
  if (order.estimatedDeliveryDate) return 'EST. DELIVERY'
  if (order.slotDate) return 'SCHEDULED FOR'
  return 'BOOKED ON'
}

function getDateValue(order) {
  if (order.estimatedDeliveryDate) return formatDate(order.estimatedDeliveryDate)
  if (order.slotDate) return formatDate(order.slotDate)
  return formatDate(order.createdAt)
}

// ---------------------------------------------------------------------------
// Sub-components — extracted to reduce cognitive complexity of OrdersPage
// ---------------------------------------------------------------------------
...
function ActiveOrderCard({ order }) {
  const deviceName = getDeviceName(order.brandRef, order.modelRef)
  const progress = calculateProgress(order.repairStatus)
  ...
  return (
    ...
    >
      <div className='bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-2xl p-5 lg:p-7 hover:border-[var(--theme-border-strong)] transition-colors'>
        ...
        {(order.symptoms?.length > 0 || order.repairTypes?.length > 0) && (
          <div className='mb-3 lg:mb-5 space-y-1.5 lg:space-y-2'>
            {order.symptoms?.length > 0 && (
              <div className='flex items-center gap-1.5 lg:gap-2 flex-wrap'>
                ...
                <div className='flex gap-1.5 lg:gap-2 flex-wrap'>
                  {order.symptoms.slice(0, 2).map((symptom) => (
                    <span
                      key={symptom.name || symptom.id || symptom._id}
            ...
            {order.repairTypes?.length > 0 && (
              <div className='flex items-center gap-1.5 lg:gap-2 flex-wrap'>
                ...
                <div className='flex gap-1.5 lg:gap-2 flex-wrap'>
                  {order.repairTypes.slice(0, 2).map((repair) => (
                    <span
                      key={repair.name || repair.id || repair._id}
        ...
        <div className='mt-4 mb-0 lg:mb-5'>
          <div className='flex items-center justify-between mb-2'>
            <span className='text-[11px] lg:text-[12px] font-bold text-[var(--theme-text-secondary)]'>
              {getStatusLabel(order.repairStatus)}
            </span>
        ...
        <div className='flex items-center gap-2 lg:gap-6 mt-4 pt-4 lg:pt-5 border-t border-[var(--theme-border)]'>
          ...
          <div className='text-right lg:flex lg:items-center lg:gap-6'>
            ...
            <button type='button' className='hidden lg:block px-5 py-2.5 bg-[var(--theme-text-primary)] text-[var(--theme-bg)] rounded-xl text-[12px] font-bold tracking-wide hover:opacity-90 transition-opacity'>
              VIEW DETAILS →
            </button>
          </div>
        </div>
...
function StatusSidebarCard({ order }) {
  return (
      ...
      <div className='space-y-2 mt-4'>
        {order.orderPlacedAt && (
          <div className='flex items-center gap-2 text-[11px]'>
            ...
            <span className='text-[var(--theme-text-secondary)]'>{formatDate(order.orderPlacedAt)}</span>
          </div>
        )}
        {order.pickupCompletedAt && (
          <div className='flex items-center gap-2 text-[11px]'>
            ...
            <span className='text-[var(--theme-text-secondary)]'>{formatDate(order.pickupCompletedAt)}</span>
          </div>
        )}
        {order.diagnosisCompletedAt && (
          <div className='flex items-center gap-2 text-[11px]'>
            <div className='w-1.5 h-1.5 rounded-full bg-green-500'></div>
            <span className='text-[var(--theme-text-tertiary)]'>Diagnosis done:</span>
            <span className='text-[var(--theme-text-secondary)]'>{formatDate(order.diagnosisCompletedAt)}</span>
          </div>
        )}
        {order.repairCompletedAt && (
          <div className='flex items-center gap-2 text-[11px]'>
            <div className='w-1.5 h-1.5 rounded-full bg-green-500'></div>
            <span className='text-[var(--theme-text-tertiary)]'>Repair done:</span>
            <span className='text-[var(--theme-text-secondary)]'>{formatDate(order.repairCompletedAt)}</span>
          </div>
        )}
        {order.estimatedDeliveryDate && !order.deliveredAt && (
          <div className='flex items-center gap-2 text-[11px]'>
            ...
            <span className='text-[var(--theme-text-secondary)]'>{formatDate(order.estimatedDeliveryDate)}</span>
          </div>
        )}
      </div>
    </div>
          ...
          order.paidAmount && (
            <div className='flex items-center justify-between'>
              ...
              <span className='text-[11px] text-red-400 font-bold'>
                {formatCurrency(order.finalCost - order.paidAmount)}
              </span>
            </div>
          )}
      </div>
...
function ServiceHistory({ orders, pastOrders, currentOrder, totalPages, currentPage, setCurrentPage }) {
  return (
    <div className='mt-6'>
      ...
      <div className='lg:hidden flex flex-col gap-3'>
        {pastOrders.map((order) => (
          <OrderHistoryRow key={order.ticketNumber} order={order} />
        ))}
      </div>
      ...
      <div className='hidden lg:block bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-2xl overflow-hidden'>
        <div className='grid grid-cols-12 gap-4 px-6 py-3 bg-[var(--theme-surface)] border-b border-[var(--theme-border)]'>
          <div className='col-span-2 text-[10px] uppercase tracking-[0.14em] text-[var(--theme-text-tertiary)] font-bold'>Order ID</div>
          <div className='col-span-4 text-[10px] uppercase tracking-[0.14em] text-[var(--theme-text-tertiary)] font-bold'>Device</div>
          <div className='col-span-3 text-[10px] uppercase tracking-[0.14em] text-[var(--theme-text-tertiary)] font-bold'>Status</div>
          <div className='col-span-2 text-[10px] uppercase tracking-[0.14em] text-[var(--theme-text-tertiary)] font-bold'>Date</div>
          <div className='col-span-1'></div>
        </div>
        {pastOrders.map((order, index) => (
          <OrderHistoryTableRow key={order.ticketNumber} order={order} />
        ))}
      </div>
...
// ---------------------------------------------------------------------------
// Main page component — cognitive complexity now well within SonarQube limit
// ---------------------------------------------------------------------------
export default function OrdersPage() {
  ...
  const activeOrder = orders.find((order) => !CLOSED_STATUSES.has(order.repairStatus))
  ...
  const renderContent = () => {
    ...
    return (
      <div className='lg:grid lg:grid-cols-12 lg:gap-6'>
        ...
        <div className='hidden lg:flex lg:col-span-5 flex-col gap-6'>
          {currentOrder && <StatusSidebarCard order={currentOrder} />}
          {currentOrder && <OrderSummaryCard order={currentOrder} />}
        </div>
