...
export default function OrderDetailPage() {
  ...
  const device =
    order &&
    [order.brandRef?.name, order.modelRef?.name].filter(Boolean).join(' ')
  ...
  return (
    <main className='min-h-[100svh] px-4 pt-[60px] pb-[40px] text-[var(--theme-text-primary)] bg-[var(--theme-bg)]'>
      <section style={{ maxWidth: 720, margin: '0 auto' }}>
        <button
          type='button'
          onClick={() => router.back()}
          className='border-0 bg-transparent text-[var(--theme-text-secondary)] hidden lg:flex items-center gap-[7px] cursor-pointer mb-6'
        >
        ...
        {error ? (
          ...
          <>
            <div className='flex items-start justify-between gap-4 mb-[22px]'>
              <div>
                <p className='text-[var(--theme-text-tertiary)] uppercase tracking-[0.12em] text-[11px] mb-2'>
                  Order ID
                </p>
              ...
              <button
                ...
                className='btn-secondary !h-[40px] !px-3'
            ...
            <div className='bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-[18px] p-[22px] mb-5 flex items-center justify-between gap-4 flex-wrap'>
              <div>
                <p className='text-[var(--theme-text-tertiary)] text-xs mb-[9px]'>
                  Current Status
                </p>
            ...
            <div className='bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-[18px] p-[22px] mb-5'>
              <h2 className='text-base font-bold mb-4 flex items-center gap-2'>
                <Package size={17} className='text-[var(--color-accent)]' />{' '}
                Repair Summary
              </h2>
              <SummaryRow label='Device' value={device || 'Device Repair'} />
              <SummaryRow
                label='Repair'
                value={repairs || 'Inspection and repair'}
              />
              <SummaryRow
                label='Pickup'
                value={`${formatDate(order.slotDate)}${order.slotTime ? `, ${order.slotTime}` : ''}`}
              />
              ...
              <SummaryRow
                ...
                value={
                  order.finalCost != null
                    ? `Rs ${Number(order.finalCost).toLocaleString('en-IN')}`
                    : 'To be confirmed'
                }
                last
              />
            ...
            {order.repairStatus === 'PENDING_ADVANCE_PAYMENT' && (
              <div className='bg-amber-400/10 text-amber-400 border border-amber-400/25 rounded-2xl p-[18px] mb-5'>
                Diagnosis is complete. Our support team will contact you
                regarding the required advance payment.
              </div>
            )}
            ...
            />
            {['PICKUP_ASSIGNED', 'PICKUP_EN_ROUTE'].includes(
              order.repairStatus,
            ) && (
              <PartnerCard
            ...
            {['DELIVERY_ASSIGNED', 'OUT_FOR_DELIVERY'].includes(
              order.repairStatus,
            ) && (
              <PartnerCard
...
function SummaryRow({ label, value, last = false }) {
  return (
    <div
      className='grid grid-cols-[92px_1fr] gap-3 py-[10px] text-[13px]'
      style={{ borderBottom: last ? 'none' : '1px solid var(--theme-divider)' }}
    >
