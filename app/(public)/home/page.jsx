...
const getDisplayStatusLabel = (status) => {
  switch (status) {
    ...
    case 'CS_CONFIRMED': return 'Booking Confirmed';
    case 'PICKUP_ASSIGNED': return 'Pickup Scheduled';
    case 'PICKUP_IN_PROGRESS': return 'Pickup En Route';
    case 'PICKUP_EN_ROUTE': return 'Pickup En Route';
    case 'DEVICE_PICKED_UP': return 'Device Picked Up';
    case 'RECEIVED_AT_CENTRE': return 'At Service Centre';
    case 'DIAGNOSIS_IN_PROGRESS': return 'Under Diagnosis';
    case 'DIAGNOSIS_COMPLETE': return 'Diagnosis Complete';
    case 'CUSTOMER_APPROVAL_PENDING': return 'Approval Pending';
    case 'CUSTOMER_APPROVED': return 'Repair Authorized';
    case 'REPAIR_IN_PROGRESS': return 'Repair in Progress';
    case 'REPAIR_COMPLETED': return 'Repair Completed';
    case 'DELIVERY_ASSIGNED': return 'Delivery Scheduled';
...
const getProgressFill = (status) => {
  switch (status) {
    ...
    case 'CS_CONFIRMED': return '30%';
    case 'PICKUP_ASSIGNED': return '40%';
    case 'PICKUP_IN_PROGRESS': return '50%';
    case 'PICKUP_EN_ROUTE': return '50%';
    case 'DEVICE_PICKED_UP': return '60%';
    case 'RECEIVED_AT_CENTRE': return '65%';
    case 'DIAGNOSIS_IN_PROGRESS': return '70%';
    case 'DIAGNOSIS_COMPLETE': return '75%';
    case 'CUSTOMER_APPROVAL_PENDING': return '80%';
    case 'CUSTOMER_APPROVED': return '85%';
    case 'REPAIR_IN_PROGRESS': return '90%';
    case 'REPAIR_COMPLETED': return '95%';
    default: return '65%';
  }
};
...
export default function HomePage() {
  ...
  const [manuals, setManuals] = useState([
    { label: 'iPhone Display Repair SOP', sub: 'Standard operating manual', Icon: BookOpen },
    { label: 'Li-ion Battery Safety SOP', sub: 'Handling and thermal safety', Icon: FileText },
    { label: 'Soldering & Micro-jumpers Guide', sub: 'Logic board diagnostic standard', Icon: History },
  ]);
    ...
    const fetchOrders = async () => {
      try {
        ...
        if (res && res.success && res.data?.orders) {
          ...
          const active = list.find(o => !['DELIVERED', 'CANCELLED'].includes(o.repairStatus));
          if (active) {
          ...
          if (latestDevice) {
            setManuals([
              { label: `${latestDevice.modelRef?.name} Screen Guide`, sub: 'Intake and glass separation SOP', Icon: BookOpen },
              { label: `${latestDevice.modelRef?.name} Battery SOP`, sub: 'Safe extraction and adhesive release', Icon: FileText },
              { label: `${latestDevice.modelRef?.name} Tear-down Index`, sub: 'Complete components map', Icon: History },
            ]);
          }
        }
      } catch (err) {
                ...
                <>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
                      ...
                      <span className="badge badge-accent">{getDisplayStatusLabel(activeOrder.repairStatus)}</span>
                    </div>
                    <h3 style={{ fontWeight: 700, fontSize: 15, color: 'var(--color-content-text)', marginBottom: 6 }}>Active Repair</h3>
                    <p style={{ fontSize: 12, color: 'var(--color-content-text-secondary)', marginBottom: 14 }}>
                      {activeOrder.brandRef?.name || ''} {activeOrder.modelRef?.name || 'Device'} — {activeOrder.repairTypes?.[0]?.name || activeOrder.symptoms?.[0]?.name || 'Device Diagnostics'}
                    </p>
                    <div style={S.progressTrack}>
                      <div style={{ ...S.progressFill, width: getProgressFill(activeOrder.repairStatus) }} />
                    </div>
                  </div>
                  <button onClick={() => router.push(`/orders/${activeOrder.ticketNumber}`)} className="btn-accent" style={{ width: '100%', height: 42, fontSize: 12, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                    Track Order
                  </button>
                </>
              ) : (
                <>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
                      ...
                      <span className="badge badge-accent" style={{ background: 'var(--color-bg-500)', color: 'var(--color-text-dim)' }}>Ready</span>
                    </div>
                    <h3 style={{ fontWeight: 700, fontSize: 15, color: 'var(--color-content-text)', marginBottom: 6 }}>Start a Repair</h3>
                    <p style={{ fontSize: 12, color: 'var(--color-content-text-secondary)', marginBottom: 14 }}>
                      No active repair in progress. Book a premium repair service now.
                    </p>
                    <div style={S.progressTrack}>
                      <div style={{ ...S.progressFill, width: '0%' }} />
                    </div>
                  </div>
                  <button onClick={handleStart} className="btn-accent" style={{ width: '100%', height: 42, fontSize: 12, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                    Book Service
                  </button>
                </>
              )}
            ...
            <div style={{ ...S.darkCard, padding: '18px 20px' }}>
              {activeOrder ? (
                <>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
                    <div>
                      ...
                      <h4 style={{ fontSize: 16, fontWeight: 800, color: 'var(--color-btn-cta-bg)' }}>
                        {activeOrder.brandRef?.name || ''} {activeOrder.modelRef?.name || 'Device'}
                      </h4>
                    </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16, paddingTop: 14, borderTop: '1px solid rgba(255,255,255,0.07)' }}>
                    <div>
                      <span style={{ ...S.mutedLabel, display: 'block', marginBottom: 2 }}>Service Ticket</span>
                      <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-btn-cta-bg)' }}>
                        #{activeOrder.ticketNumber}
                      </span>
                    </div>
                    <button onClick={() => router.push(`/orders/${activeOrder.ticketNumber}`)} style={{ background: 'var(--color-btn-cta-bg)', color: 'var(--color-btn-cta-text)', border: 'none', borderRadius: 10, padding: '9px 18px', fontSize: 12, fontWeight: 700, letterSpacing: '0.04em', cursor: 'pointer' }}>
                      Track Order
                    </button>
                  </div>
                </>
                ...
                <>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
                    <div>
                      <span style={{ ...S.mutedLabel, display: 'block', marginBottom: 4 }}>Repair Service</span>
                      <h4 style={{ fontSize: 16, fontWeight: 800, color: 'var(--color-btn-cta-bg)' }}>Start a Repair</h4>
                    </div>
                    <span className="badge badge-accent" style={{ marginTop: 2, background: 'var(--color-bg-500)', color: 'var(--color-text-dim)' }}>Ready</span>
                  </div>
                  <div style={S.progressTrack}>
                    <div style={{ ...S.progressFill, width: '0%' }} />
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16, paddingTop: 14, borderTop: '1px solid rgba(255,255,255,0.07)' }}>
                    <div>
                      <span style={{ ...S.mutedLabel, display: 'block', marginBottom: 2 }}>Status</span>
                      <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-btn-cta-bg)' }}>No active repair</span>
                    </div>
                    <button onClick={handleStart} style={{ background: 'var(--color-btn-cta-bg)', color: 'var(--color-btn-cta-text)', border: 'none', borderRadius: 10, padding: '9px 18px', fontSize: 12, fontWeight: 700, letterSpacing: '0.04em', cursor: 'pointer' }}>
                      Book Service
                    </button>
                  </div>
                </>
